...
(function () {
    ...
    // ---- WebSocket Interceptor & Mock Server ----
    (function () {
        var _origWS = window.WebSocket;
        window.WebSocket = function (url, protocols) {
            console.log('[Web] Intercepted WS connection to:', url);
            if (url.indexOf('20111') !== -1) {
                console.log('[Web] MOCKING Link Server connection for', url);
                var fakeWs = {
                    url: url,
                    readyState: 1, // OPEN
                    send: function (data) {
                        console.log('[Fake WS Send] to ' + url + ' | Data:', data);
                        try {
                            var parsed = JSON.parse(data);
                            var response = { jsonrpc: "2.0", id: parsed.id };

                            // Mock basic OpenBlock Link responses to keep GUI happy
                            if (parsed.method === 'ping') {
                                response.result = "pong";
                            } else if (parsed.method === 'discover') {
                                // Return a fake serial port
                                response.result = [{ path: "COM_FAKE", manufacturer: "Fake Device" }];
                            } else if (parsed.method === 'connect') {
                                response.result = "connected";
                            } else if (parsed.method === 'upload') {
                                console.log('--- FOUND UPLOAD COMMAND ---', parsed);
                                response.result = "Upload sequence intercepted!";
                            } else {
                                response.result = "ok";
                            }
                            ...
                            setTimeout(function () {
                                if (typeof fakeWs.onmessage === 'function') {
                                    fakeWs.onmessage({ data: JSON.stringify(response) });
                                }
                            }, 50);
                        } catch (e) {
                            console.error('Fake WS parse error', e);
                        }
                    },
                    close: function () { console.log('[Fake WS] closed'); },
        ...
        console.log('[TUbitBlock Web] WebSocket Mock Server installed.');
    })();
